...
def single_post(slug):
    ...
    return render_template('blog/post_content.html', post=post, search_form=search_form)
...
def search_blog():
    ...
    # اگر داخل فیلتر پارامترهای گرفته شده را مستقیما بنویسیم فقط پست هایی را نشان میدهد
    # که متغیر در هر سه قسمت مقاله وجود داشته باشد که این روش غلط است
    found_posts = Post.query.filter(
        or_(title_cond, summary_cond, content_cond)).all()
...
def single_category(slug):
    ...
    # اگر کتگوری پیدا شد تمام پست ههای مربوط به ان را کتگوری را طبق رابطه در مدل پیدا میکند
